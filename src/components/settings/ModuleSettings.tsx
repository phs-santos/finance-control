...
export const ModuleSettings = ({ onUpdate }: ModuleSettingsProps) => {
	...
	const handleToggle = async (moduleId: string) => {
		try {
			...
			toast({ title: "Módulo atualizado", description: `${module?.name} foi ${newState ? 'ativado' : 'desativado'}` });
			onUpdate();
		} catch (error) {
			toast({ title: "Erro", description: "Falha ao atualizar o módulo", variant: "destructive" });
		}
	};
